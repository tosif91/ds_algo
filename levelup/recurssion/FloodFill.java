...
public class FloodFill {
    ...
    public static void main(String[] args) {
        ...
        String sp = floodfillWithJumpsShortestPath(maze, 0, 0, dir, dirs, "");
    ...
    public static String floodfillWithJumpsShortestPath(int[][] maze, int sr, int sc, int[][] dir, String[] dirS,
            String psf) {
        int dr = maze.length - 1;
        int dc = maze[0].length - 1;
        if (sr == dr && sc == dc) {
        ...
        for (int d = 0; d < dir.length; d++) {
            for (int rad = 1; rad < Math.max(dr, dc); rad++) {
                ...
                int c = sc + rad * dir[d][1];
                if (r >= 0 && c >= 0 && r <= dr && c <= dc && maze[r][c] != 1) {
                    psf = floodfillWithJumpsShortestPath(maze, r, sc, dir, dirS, psf+dirS[d]);
                    if (psf.length() < maxPath) {
                        maxPath = psf.length();
                        shortestPath = psf;
                    }
                }
